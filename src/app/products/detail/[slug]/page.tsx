...
export async function generateStaticParams() {
  let posts: TResponseProducts = await fetch(
    "https://vhg.vovantrong.xyz/products"
  ).then((res) => res.json());
...
export default async function PageProductDetail({
  ...
  const dataProductSuggest:TResponseProducts = await getProductsWithType(type.name,6)
  return <ProductDetail data={dataProductDetail.data} dataSuggest={dataProductSuggest.data} />;
}
